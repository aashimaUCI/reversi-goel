...
*/
    ...
    */
    ...
    socket.on('play_token', (payload) => {
        ...
        /* make sure current turn is coming from the expected player */
        if (
            ...
            let response = {
                result: 'fail',
                message: 'play_token played the right color but with wrong player'
            }
            socket.emit('play_token_response', response);
            serverLog('play_token command failed', JSON.stringify(response));
            return;
...
function check_line_match(color, dr, dc, r, c, board) {
    ...
    /* make sure we didn't walk off the board*/
    if ((r + dr < 0) || (r + dr > 7)) {
        return false;
    }
    if ((c + dc < 0) || (c + dc > 7)) {
...
function adjacent_support(who, dr, dc, r, c, board) {
    ...
    /* check to make sure if adjacent support is on the board */
    if ((r + dr < 0) || (r + dr > 7)) {
        return false;
    }
    if ((c + dc < 0) || (c + dc > 7)) {
